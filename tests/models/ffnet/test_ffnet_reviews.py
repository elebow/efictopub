...
class TestFFNetReviews:
    ...
    def test_as_comment(self):
        assert [r.as_comment().author for r in self.reviews] \
            == ['User 201', 'User 202', 'User 203', 'User 204', 'User 205', 'User 206']

        assert [r.as_comment().date_published.timestamp() for r in self.reviews] \
            == [1525044182, 1502099520, 1496473762, 1494499329, 1494441199, 1492430074]

        assert [r.as_comment().text for r in self.reviews] \
            == ['Review 1 content',
                'Review 2 content',
